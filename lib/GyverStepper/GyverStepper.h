...
template <GS_driverType _DRV> class GStepper {
public:
  // конструктор
  GStepper(long stepsPerRev, int8_t pin1, int8_t pin2, int8_t pin3 = -1, int8_t pin4 = -1, int8_t pin5 = -1)
  ...
  bool tick() {
    ...
    if (_workState && !_curMode && _accel != 0 && _maxSpeed > _MIN_STEPPER_SPEED)
      planner();
#endif
    // при плавном разгоне в KEEP_SPEED
    if (_smoothStart && _curMode)
      smoothSpeedPlanner();
  ...
  void setSpeed(float speed, bool smooth = false) {
    // 1 шаг в час минимум
    _speed = speed;
    if (abs(_speed) < MIN_STEPPER_SPEED)
      _speed = MIN_STEPPER_SPEED * _sign(_speed);

    if (smooth && abs(speed) > _MIN_STEPPER_SPEED) { // плавный старт
      if (_accelSpeed == _speed)
        return; // скорости совпадают? Выходим
      _smoothStart = true;
#ifdef __AVR__
      ...
      if (_speed == 0) {
        brake();
        return;
      } // скорость 0? Отключаемся и выходим
      _accelSpeed = _speed;
  ...
  void enable() {
    ...
    if (_DRV == STEPPER4WIRE || _DRV == STEPPER4WIRE_HALF)
      step(); // подадим прошлый сигнал на мотор, чтобы вал зафиксировался
    if (_enPin != -1)
      digitalWrite(_enPin, _enDir);
  }
  ...
  uint32_t getMinPeriod() {
    if (_curMode == KEEP_SPEED)
      return abs(1000000.0 / _speed);
    else
      return (1000000.0 / _maxSpeed);
  }
...
private:
  ...
  void planner() {
    if (micros() - _plannerTime >= _plannerPrd) {
      ...
      if (abs(_accelSpeed) > _MIN_STEPPER_SPEED)
        stepTime = abs(1000000.0 / _accelSpeed); // ограничение на мин. скорость
      else
        stepTime = _MAX_STEP_PERIOD;
